...
const seedData = async () => {
    try {
        ...
        const lessons = await Lesson.insertMany([
            { courseId: courses[0]._id, lessonName: 'Variables and Data Types', content: 'Introduction to variables', position: 1 },
            { courseId: courses[1]._id, lessonName: 'Async Programming', content: 'Understanding asynchronous code', position: 1 }
        ]);
        ...
        await Recommendation.insertMany([
            { userId: users[0]._id, courseId: courses[0]._id, recommendedLessonId: lessons[0]._id, reason: 'Start with basics' },
            { userId: users[1]._id, courseId: courses[1]._id, recommendedLessonId: lessons[1]._id, reason: 'Improve async understanding' }
        ]);
